...
{/* TODO: figure out homework autograding mechanism */}
...
### Using a formatter (optional)

While linters are useful for finding problems, they typically don't deal with code formatting and visual style. Formatters instead focus on whitespace, newline placement, and other issues that impact the readability of code. We recommend using [Prettier](https://prettier.io/), which can handle many file types including JS and TS. Similar to ESLint, Prettier uses a `.prettierrc.json` file for configuration.

<Alert type="info" mdx>
  We recommend using ESLint and Prettier together. The `eslint-config-prettier` package ([found here](https://www.npmjs.com/package/eslint-config-prettier)) will disable any ESlint rules which conflict or are redundant with Prettier.
  
  In VSCode, you can use the [Prettier](https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode) and [ESLint](https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint) plugins at the same time. We recommend enabling the "Format on save" setting, and making Prettier the default formatter for maximum convenience.
